...
export function AddMeasurementDialog({ children, open, onOpenChange, customerId }: AddMeasurementDialogProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const { error } = await supabase.from('measurements').insert(measurementToInsert)

    if (error) {
        toast({ variant: "destructive", title: "Error adding measurement", description: error.message })
        setIsSubmitting(false)
        return
    }
    ...
    toast({
        ...
        description: `A new measurement record has been saved.`,
    })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[800px] grid-rows-[auto,1fr,auto] max-h-[90vh] p-0">
        <DialogHeader className="p-6 pb-4">
          ...
          <DialogDescription>
            Enter new measurements. All values are in inches.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="p-6 pt-4 border-t sm:justify-start">
            <Button type="submit" form="add-measurement-form" disabled={isSubmitting}>
